  ...
  return <main style={{minHeight:'100vh',background:'#eef3f6',padding:'16px 12px 110px',fontFamily:'Inter,system-ui,sans-serif',color:'#102033'}}>
    <section style={{maxWidth:760,margin:'0 auto'}}>
      <div style={{background:'#102033',color:'#fff',borderRadius:20,padding:'16px',marginBottom:12}}>
        <small style={{display:'block',opacity:.72,fontWeight:800,letterSpacing:'.06em'}}>{lang === 'fr' ? 'TRAJET EN COURS' : 'TRAJÈ AP FÈT'}</small>
        <h1 style={{margin:'6px 0 2px',fontSize:24}}>{ride.destination_address}</h1>
        <div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:10}}>
          <span style={{background:'#1c3148',borderRadius:999,padding:'8px 10px',fontWeight:800}}>{ride.estimated_distance_km == null ? '—' : `${Number(ride.estimated_distance_km).toFixed(1)} km`}</span>
          <span style={{background:'#1c3148',borderRadius:999,padding:'8px 10px',fontWeight:800}}>⏱ {ride.estimated_duration_min == null ? '—' : `${Math.round(Number(ride.estimated_duration_min))} min`}</span>
        </div>
      </div>
      ...
      <button onClick={() => void finishRide()} disabled={busy} style={{width:'100%',marginTop:14,border:0,borderRadius:16,padding:'16px',background:'#0d7b61',color:'#fff',fontSize:17,fontWeight:900}}>
        {busy ? (lang === 'fr' ? 'Finalisation…' : 'N ap fini…') : (lang === 'fr' ? 'Terminer le trajet' : 'Fini trajè a')}
      </button>
    </section>
  </main>
}
