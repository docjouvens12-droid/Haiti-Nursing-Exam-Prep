...
export default function DriverDashboardPage() {
  ...
  {menuOpen && <div className="menuBackdrop" onClick={() => setMenuOpen(false)}>
    <aside className="drawer" onClick={(e) => e.stopPropagation()}>
      ...
      {vehicle && <div className="menuSection"><h3>{t.vehicle}</h3><p><span>Modèle</span><b>{vehicle.make} {vehicle.model}</b></p><p><span>{t.color}</span><b>{vehicle.color || '—'}</b></p><p><span>{t.plate}</span><b>{vehicle.plate_number}</b></p></div>}
