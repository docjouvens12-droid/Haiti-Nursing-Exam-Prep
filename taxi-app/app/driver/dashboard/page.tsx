...
export default function DriverDashboardPage() {
  ...
  async function init() {
    ...
    const { data: driver } = await supabase.from('driver_profiles').select('status,is_online,average_rating,total_rides').eq('user_id', auth.user.id).maybeSingle()
  ...
  async function toggleOnline() {
    ...
    if (userId) {
      const { data: driver, error: refreshError } = await supabase
        ...
        .select('is_online,average_rating,total_rides')
        .eq('user_id', userId)
        .maybeSingle()
      if (!refreshError && driver) {
        confirmed = Boolean(driver.is_online)
        setAverageRating(Number(driver.average_rating ?? 0))
        setTotalRides(Number(driver.total_rides ?? 0))
      }
    }
