...
const copy = {
  fr: {
    title: 'Dossiers d’intervention', subtitle: 'Attribuez, traitez et auditez chaque alerte de sécurité', back: 'Sécurité', loading: 'Chargement…', denied: 'Accès réservé aux administrateurs.', open: 'Ouverts', resolved: 'Résolus', take: 'Prendre en charge', mine: 'Pris en charge par vous', assigned: 'Pris en charge par', unassigned: 'Non attribué', timeline: 'Journal d’intervention', noActions: 'Aucune action enregistrée.', callPassenger: 'Appeler le passager', callDriver: 'Appeler le chauffeur', acknowledge: 'Reconnaître', resolve: 'Résoudre', reopen: 'Rouvrir', note: 'Note admin', saveNote: 'Enregistrer la note', route: 'Trajet', status: 'Statut', detected: 'Détectée', driver: 'Chauffeur', passenger: 'Passager', error: 'Impossible de terminer cette action.'
  },
  ht: {
    title: 'Dosye entèvansyon', subtitle: 'Pran, trete epi verifye chak alèt sekirite', back: 'Sekirite', loading: 'N ap chaje…', denied: 'Se administratè sèlman ki gen aksè.', open: 'Ouvè', resolved: 'Rezoud', take: 'Pran dosye a', mine: 'Se ou ki pran dosye sa a', assigned: 'Dosye a sou', unassigned: 'Poko gen admin', timeline: 'Jounal entèvansyon', noActions: 'Poko gen okenn aksyon anrejistre.', callPassenger: 'Rele kliyan', callDriver: 'Rele chofè', acknowledge: 'Rekonèt', resolve: 'Rezoud', reopen: 'Relouvri', note: 'Nòt admin', saveNote: 'Anrejistre nòt', route: 'Trajè', status: 'Estati', detected: 'Detekte', driver: 'Chofè', passenger: 'Kliyan', error: 'Nou pa ka fini aksyon sa a.'
  }
}
...
export default function AdminSafetyCasesPage() {
  ...
  async function loadEvents(showBusy = true) {
    ...
    const { data, error } = await supabase.from('ride_safety_events')
      .select('id,ride_id,passenger_id,event_type,severity,acknowledged_at,resolved_at,admin_note,assigned_admin_id,assigned_at,created_at')
  ...
  async function selectEvent(row: SafetyEvent) {
    ...
    const [{ data: d }, _] = await Promise.all([
  ...
  return <main className="page"><section className="card">
    ...
    <h1>{t.title}</h1>
    <div className="stats"><div><b>{openCount}</b><span>{t.open}</span></div><div><b>{resolvedCount}</b><span>{t.resolved}</span></div></div>
    {message && <div className="message">{message}</div>}
    {busy && <p className="muted">{t.loading}</p>}
    <div className="grid">
      <div className="cases">{events.map(e => <button key={e.id} className={`case ${selected?.id===e.id?'active':''}`} onClick={() => void selectEvent(e)}><div><strong>{e.passenger_name || t.passenger}</strong><span>{e.event_type.replaceAll('_',' ')}</span></div><small>{e.assigned_admin_name ? `${t.assigned} ${e.assigned_admin_name}` : t.unassigned}<br/>{dateLabel(e.created_at)}</small></button>)}</div>
      <div className="workspace">{!selected ? <div className="empty">← {t.title}</div> : <>
        <div className="caseHead"><div><small>{selected.event_type.replaceAll('_',' ')}</small><h2>{selected.passenger_name || t.passenger}</h2></div><span className={selected.resolved_at?'done':'openState'}>{selected.resolved_at?t.resolved:t.open}</span></div>
