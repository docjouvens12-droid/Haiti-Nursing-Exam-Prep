...
export default function PassengerDashboardQuickInfo() {
  useEffect(() => {
    ...
    const build = async () => {
      ...
      if (!sheet || !greeting || sheet.querySelector('.passenger-quick-wrap')) return
      ...
      const eyebrow = greeting.querySelector<HTMLElement>('.eyebrow')
      ...
      favorites.forEach(({ key, label }) => {
        ...
        button.addEventListener('click', () => {
          if (saved) { setDestination(saved); note.textContent = ''; return }
          const inputs = Array.from(document.querySelectorAll<HTMLInputElement>('.route-card input'))
      ...
      greeting.insertAdjacentElement('afterend', wrap)
    }
    ...
    return () => {
      mounted = false
      observer.disconnect()
      document.querySelector('.passenger-quick-wrap')?.remove()
      document.getElementById(styleId)?.remove()
    }
  }, [])
