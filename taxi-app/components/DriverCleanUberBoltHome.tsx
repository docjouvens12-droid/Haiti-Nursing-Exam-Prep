...
export default function DriverCleanUberBoltHome(){
  ...
  useEffect(()=>{
    ...
    const install=()=>{
      const shell=document.querySelector<HTMLElement>('.drv2-shell')
      const topbar=shell?.querySelector<HTMLElement>('.drv2-topbar')
      if(!shell||!topbar){setTarget(null);return}
      let mount=shell.querySelector<HTMLElement>('.dcu-home-root')
      if(!mount){
        mount=document.createElement('div')
        mount.className='dcu-home-root'
  ...
  return createPortal(<>
    <style>{`
      body:has(.dcu-home-root){background:#f5f7f6!important}
      body:has(.dcu-home-root) .drv2-shell{max-width:560px!important;background:#f5f7f6!important}
      body:has(.dcu-home-root) .drv2-hero{display:none!important}
      body:has(.dcu-home-root) .drv2-status-card{background:#fff!important;border:1px solid #e2e9e6!important;border-radius:22px!important;box-shadow:0 8px 24px rgba(16,32,51,.08)!important;margin-top:12px!important}
      body:has(.dcu-home-root) .drv2-requests{margin-top:18px!important}
      body:has(.dcu-home-root) .drv2-refresh{width:46px!important;height:46px!important;padding:0!important;border-radius:50%!important;font-size:0!important;display:grid!important;place-items:center!important;background:#fff!important;border:1px solid #dce7e3!important;color:#0f705a!important;box-shadow:0 6px 18px rgba(16,32,51,.08)!important}
      body:has(.dcu-home-root) .drv2-refresh::before{content:'↻';font-size:22px;font-weight:900}
      body:has(.dcu-home-root) .drv2-refresh:disabled::before{animation:dcu-spin .8s linear infinite}
      @keyframes dcu-spin{to{transform:rotate(360deg)}}
      .dcu-home{margin:14px 0 4px}.dcu-map-shell{height:300px;border-radius:26px;overflow:hidden;position:relative;background:#eaf1ef;border:1px solid #dce7e3;box-shadow:0 10px 28px rgba(16,32,51,.08)}.dcu-map{width:100%;height:100%}.dcu-map-label{position:absolute;left:14px;top:14px;z-index:3;background:rgba(255,255,255,.96);backdrop-filter:blur(8px);border-radius:999px;padding:9px 13px;font-size:12px;font-weight:900;color:#102033;box-shadow:0 4px 14px rgba(0,0,0,.08);border:1px solid rgba(15,112,90,.12)}.dcu-map-fallback{height:100%;display:grid;place-items:center;text-align:center;padding:20px;color:#617281;font-weight:800}.dcu-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin-top:11px}.dcu-stat{background:#fff;border:1px solid #e0e8e5;border-radius:18px;padding:13px 9px;min-width:0;box-shadow:0 5px 16px rgba(16,32,51,.04)}.dcu-stat small,.dcu-stat strong{display:block}.dcu-stat small{font-size:9px;color:#7d8b98;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.dcu-stat strong{margin-top:6px;font-size:14px;color:#102033}.dcu-stat strong span{margin-right:3px}.dcu-marker{width:42px;height:42px;border-radius:50%;display:grid;place-items:center;background:#fff;border:3px solid #0f705a;box-shadow:0 5px 14px rgba(0,0,0,.2);font-size:21px}
    `}</style>
    <div className="dcu-home">
