...
export default function PassengerRideCompletion() {
  ...
  return <div className="receiptBackdrop" role="dialog" aria-modal="true" aria-label={isHt ? 'Trajè fini' : 'Trajet terminé'}>
    <section className="receiptCard">
      <div className="check">✓</div>
      <p className="eyebrow">{isHt ? 'TRAJÈ FINI' : 'TRAJET TERMINÉ'}</p>
      <h2>{isHt ? 'Mèsi paske ou te vwayaje ak Taxi Platform Haiti' : 'Merci d’avoir voyagé avec Taxi Platform Haiti'}</h2>
      ...
      <div className="ratingBox">
        ...
        <div className="stars" aria-label={isHt ? 'Evalyasyon sou senk etwal' : 'Évaluation sur cinq étoiles'}>
          {[1,2,3,4,5].map((n) => <button key={n} disabled={rated || busy} className={n <= stars ? 'selected' : ''} onClick={() => setStars(n)} aria-label={`${n}`}>★</button>)}
        </div>
        ...
        {!rated && <button className="submit" disabled={busy || stars === 0} onClick={submitRating}>{busy ? (isHt ? 'N ap voye…' : 'Envoi…') : (isHt ? 'Voye evalyasyon mwen' : 'Envoyer mon évaluation')}</button>}
      </div>

      <button className="close" onClick={closeReceipt}>{rated ? (isHt ? 'Fini' : 'Terminer') : (isHt ? 'Pita' : 'Plus tard')}</button>
    </section>
    ...
    <style jsx>{`
      ...
      .submit,.close{width:100%;border:0;border-radius:13px;padding:13px 14px;font-size:14px;font-weight:900;cursor:pointer}.submit{margin-top:9px;background:#1b70eb;color:#fff}.submit:disabled{opacity:.45;cursor:not-allowed}.close{margin-top:8px;background:#eef2f5;color:#304357}.success,.error{margin:8px 0 0;font-size:12px;line-height:1.35}.success{color:#0c7659;font-weight:800}.error{color:#a22d2d;font-weight:800}
