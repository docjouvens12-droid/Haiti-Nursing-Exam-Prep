...
export default function DriverFinalMenuStable(){
  ...
  const earnings=useMemo(()=>{
    const map=new Map(payments.map(p=>[p.ride_id,p]));
    return rides.reduce((a,r)=>{const p=map.get(r.id);a.gross+=Number(p?.amount_htg||r.final_fare_htg||0);a.fee+=Number(p?.platform_fee_htg||0);a.net+=Number(p?.driver_net_htg||0);return a},{gross:0,fee:0,net:0})
  },[rides,payments])
  const ht=lang==='ht'
  ...
  return createPortal(<div className="dfm-wrap" onClick={e=>e.stopPropagation()} onPointerDown={e=>e.stopPropagation()}>
    ...
    <details className="dfm-section"><Summary label={ht?'Èd':'Aide'} plus/><div className="dfm-body">{(ht?[["Trajè ak demann","Rete sou liy pou resevwa demann epi verifye pwen depa ak destinasyon an."],["Peman","Chwazi yon sèl metòd payout: MonCash oswa NatCash."],["Kont ak veyikil","Ou ka anrejistre epi modifye enfòmasyon Profil ak Veyikil ou."],["Sekirite","Pa kòmanse yon trajè si enfòmasyon yo pa koresponn oswa sitiyasyon an pa sanble an sekirite."]]:[["Trajets et demandes","Restez en ligne pour recevoir les demandes et vérifiez le départ et la destination."],["Paiements","Choisissez un seul mode de versement : MonCash ou NatCash."],["Compte et véhicule","Vous pouvez enregistrer et modifier votre Profil et votre Véhicule."],["Sécurité","Ne commencez pas un trajet si les informations ne correspondent pas ou si la situation semble dangereuse."]]).map(([h,p])=><div className="dfm-help-card" key={h}><strong>{h}</strong><p>{p}</p></div>)}</div></details>
  </div>,target)
}
