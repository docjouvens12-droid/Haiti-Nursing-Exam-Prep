...
export default function PassengerRideStatusFlow() {
  ...
  const title = status === 'accepted'
    ...
    : status === 'driver_arriving'
      ? (ht ? 'Chofè a ap vini pran ou' : 'Le chauffeur arrive vers vous')
      : status === 'in_progress'
  ...
  const subtitle = status === 'accepted'
    ? (ht ? 'Gade mini-map la pou pozisyon chofè a parapò ak ou.' : 'Suivez la mini-carte pour voir le chauffeur par rapport à vous.')
    : status === 'driver_arriving'
      ? (ht ? 'Chofè a ap pwoche kote pou pran ou.' : 'Le chauffeur se rapproche de votre point de prise en charge.')
      : status === 'in_progress'
  ...
  return createPortal(
    <div className={`movi-passenger-flow-card ${isTerminal ? 'terminal' : ''}`} aria-live="polite">
      <style>{`
        ...
        .movi-passenger-flow-icon{width:42px;height:42px;border-radius:14px;display:grid;place-items:center;background:#e6f5ef;font-size:21px;flex:0 0 auto}
        .movi-passenger-flow-copy{min-width:0;flex:1}.movi-passenger-flow-copy strong{display:block;color:#10243a;font-size:15px;line-height:1.25;font-weight:900}.movi-passenger-flow-copy small{display:block;color:#6d7e77;font-size:11px;line-height:1.45;margin-top:4px}
      ...
      <div className="movi-passenger-flow-head">
        <span className="movi-passenger-flow-icon">{status === 'completed' ? '✅' : status === 'cancelled' ? '✕' : '🚕'}</span>
