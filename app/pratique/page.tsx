...
export default async function Pratique({ searchParams }: { searchParams: Promise<{ categorie?: string; annee?: string }> }) {
  ...
  const categories = Array.from(new Set((categoriesData ?? []).map((x) => x.categorie).filter(Boolean))).sort();
  ...
  return (
    <main className="container page">
      ...
      {!questions || questions.length === 0 ? <div className="card"><h2>Aucune question trouvée</h2><p className="muted">Modifiez les filtres ou importez davantage de questions.</p></div> : <QuestionInteractiveAvancee questions={questions} />}
    </main>
  );
}
