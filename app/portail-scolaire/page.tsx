...
export default function PortailScolairePage(){
  ...
  const addTeacher=(formData:FormData)=>{
    const name=String(formData.get('name')||'').trim()
    const subject=String(formData.get('subject')||'').trim()
    const classes=String(formData.get('classes')||'').trim()
    if(!name||!subject||!classes) return
    const nextId='ENS-'+String(teachers.length+1).padStart(3,'0')
    setTeachers([...teachers,{id:nextId,name,subject,classes}]);setAddingTeacher(false)
  }
  const saveTeacher=(formData:FormData)=>{
    if(editingTeacher===null) return
    const next=[...teachers]
    next[editingTeacher]={...next[editingTeacher],name:String(formData.get('name')||''),subject:String(formData.get('subject')||''),classes:String(formData.get('classes')||'')}
    setTeachers(next);setEditingTeacher(null)
  }
  ...
  return <main className="ps-page">
    ...
    <div className="wrap">
      ...
      {role==='direction' && <>
        <Back onClick={directionView==='home'?goHome:()=>{setDirectionView('home');setEditing(null);setEditingTeacher(null);setAddingTeacher(false)}}/><div style={{height:12}}/>
        {directionView==='home' && <><section className="card"><h2>{ht?'Tablo bò pou Direksyon an':'Tableau de bord de la Direction'}</h2><div className="stats"><div className="stat"><strong>{students.length}</strong><small>{ht?'Elèv':'Élèves'}</small></div><div className="stat"><strong>{teachers.length}</strong><small>{ht?'Pwofesè':'Enseignants'}</small></div><div className="stat"><strong>2</strong><small>{ht?'Nòt an atant':'Notes en attente'}</small></div><div className="stat"><strong>2026–2027</strong><small>{ht?'Ane akademik':'Année académique'}</small></div></div></section><section className="card"><h3>{ht?'Aksyon rapid':'Actions rapides'}</h3><div className="bigMenu"><button className="menuBtn" onClick={()=>setDirectionView('students')}>👥 {ht?'Jere elèv':'Gérer les élèves'}</button><button className="menuBtn" onClick={()=>setDirectionView('teachers')}>👩🏽‍🏫 {ht?'Jere pwofesè':'Gérer les enseignants'}</button><button className="menuBtn" onClick={()=>setDirectionView('classes')}>🏫 {ht?'Klas & seksyon':'Classes & sections'}</button><button className="menuBtn" onClick={()=>setDirectionView('subjects')}>📚 {ht?'Matiyè':'Matières'}</button><button className="menuBtn" onClick={()=>setDirectionView('pending')}>⏳ {ht?'Nòt an atant':'Notes en attente'}</button><button className="menuBtn" onClick={()=>setDirectionView('publish')}>📄 {ht?'Pibliye bilten':'Publier les bulletins'}</button></div></section></>}
        ...
        {directionView==='teachers' && <section className="card"><h2>{ht?'Jere pwofesè':'Gérer les enseignants'}</h2><button className="btn" onClick={()=>{setAddingTeacher(true);setEditingTeacher(null)}}>➕ {ht?'Ajoute yon pwofesè':'Ajouter un enseignant'}</button>{addingTeacher&&<form action={addTeacher} style={{marginTop:18}}><h3>{ht?'Ajoute yon pwofesè':'Ajouter un enseignant'}</h3><div className="grid2"><div><label>{ht?'Non konplè':'Nom complet'}</label><input name="name" required/></div><div><label>{ht?'Matiyè':'Matière'}</label><input name="subject" required/></div><div><label>{ht?'Klas yo asiyen':'Classes assignées'}</label><input name="classes" required/></div></div><div className="row" style={{marginTop:12}}><button className="btn" type="submit">{ht?'Anrejistre':'Enregistrer'}</button><button className="btn secondary" type="button" onClick={()=>setAddingTeacher(false)}>{ht?'Anile':'Annuler'}</button></div></form>}<div className="teacherList">{teachers.map((t,i)=><div className="teacherCard" key={t.id}><div className="teacherName">{t.name}</div><div className="muted">{t.id} • {t.subject}</div><div className="muted">{t.classes}</div><div className="row" style={{marginTop:10}}><button className="btn" onClick={()=>{setEditingTeacher(i);setAddingTeacher(false)}}>{ht?'Modifye':'Modifier'}</button><button className="btn danger" onClick={()=>removeTeacher(i)}>{ht?'Retire':'Retirer'}</button></div>{editingTeacher===i&&<form action={saveTeacher} style={{marginTop:14}}><div className="grid2"><div><label>{ht?'Non konplè':'Nom complet'}</label><input name="name" defaultValue={t.name}/></div><div><label>{ht?'Matiyè':'Matière'}</label><input name="subject" defaultValue={t.subject}/></div><div><label>{ht?'Klas yo asiyen':'Classes assignées'}</label><input name="classes" defaultValue={t.classes}/></div></div><div className="row" style={{marginTop:12}}><button className="btn" type="submit">{ht?'Anrejistre':'Enregistrer'}</button><button className="btn secondary" type="button" onClick={()=>setEditingTeacher(null)}>{ht?'Anile':'Annuler'}</button></div></form>}</div>)}</div></section>}
      ...
      {role==='teacher' && <>
        ...
        {teacherView==='summary' && <section className="card"><h2>{ht?'Rezime klas':'Résumé de classe'}</h2><div className="stats"><div className="stat"><strong>{students.length}</strong><small>{ht?'Elèv':'Élèves'}</small></div><div className="stat"><strong>2</strong><small>{ht?'Nòt voye':'Notes envoyées'}</small></div><div className="stat"><strong>81.5%</strong><small>{ht?'Mwayèn':'Moyenne'}</small></div><div className="stat"><strong>1</strong><small>{ht?'Matiyè':'Matière'}</small></div></div></section>}
      </>}
