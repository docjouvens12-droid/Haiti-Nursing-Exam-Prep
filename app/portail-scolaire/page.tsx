  ...
  return <main className="ps-page">
    ...
    <div className="wrap">
      ...
      {role==='teacher' && <>
        ...
        {teacherView==='home' && <section className="card"><h2>{ht?'Tablo bò Pwofesè':'Tableau de bord de l’Enseignant'}</h2><div className="menu"><button className="menuBtn" onClick={()=>setTeacherView('grades')}>📝 {ht?'Antre nòt':'Saisir les notes'}</button><button className="menuBtn" onClick={()=>setTeacherView('sent')}>📤 {ht?'Nòt mwen voye':'Notes envoyées'}</button><button className="menuBtn" onClick={()=>setTeacherView('classes')}>🏫 {ht?'Klas mwen yo':'Mes classes'}</button><button className="menuBtn" onClick={()=>setTeacherView('summary')}>📊 {ht?'Rezime klas':'Résumé de classe'}</button></div></section>}
