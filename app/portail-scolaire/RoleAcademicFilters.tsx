...
export default function RoleAcademicFilters(){
 ...
 useEffect(()=>{
  ...
  const onYear=(e:Event)=>setYear((e as CustomEvent<{year:string}>).detail?.year||currentStoredYear())
  document.addEventListener('change',onLang,true);window.addEventListener('school-academic-year-change',onYear)
  return()=>{observer.disconnect();document.removeEventListener('change',onLang,true);window.removeEventListener('school-academic-year-change',onYear);authListener.subscription.unsubscribe()}
 },[])
