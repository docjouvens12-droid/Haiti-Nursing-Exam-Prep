...
export default function StudentWeightedBulletin(){
 ...
 const html=()=>`<!doctype html><html><head><meta charset="utf-8"><title>${ht?'Bilten mwen':'Mon bulletin'}</title><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#182433;font-size:12px}.school{text-align:center;font-weight:800;color:#0f4c81;margin-bottom:8px}h1{text-align:center}.info{display:grid;grid-template-columns:1fr 1fr;gap:6px 18px;border:1px solid #d8e2ea;padding:10px;margin-bottom:14px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #d8e2ea;padding:7px}th{background:#eef4f8}td:nth-child(n+2),th:nth-child(n+2){text-align:right}.general{margin-top:14px;border:2px solid #0f4c81;padding:10px 12px;display:flex;justify-content:space-between;font-size:16px;font-weight:800}.validation{margin-top:28px;border:1px solid #9fb4c8;border-radius:10px;padding:14px}.validation-title{font-weight:900;color:#0f4c81;margin-bottom:20px}.validation-grid{display:grid;grid-template-columns:1fr 1fr;gap:26px}.validation-field{padding-top:46px;border-bottom:1px solid #374151}.validation-label{font-weight:800;padding-top:6px}.date-field{grid-column:1/-1;margin-top:8px}.footer{text-align:center;margin-top:22px;color:#667;font-size:9px}</style></head><body><div class="school">PORTAIL SCOLAIRE HAÏTI</div><h1>${ht?'Bilten — Trimès':'Bulletin — Trimestre'} ${trimester}</h1><div class="info"><div><b>${ht?'Elèv':'Élève'}:</b> ${esc(student?.name||'—')}</div><div><b>${ht?'Ane akademik':'Année scolaire'}:</b> ${esc(student?.year||'—')}</div><div><b>${ht?'Klas / Nivo':'Classe / Niveau'}:</b> ${esc(student?.level||'—')}</div><div><b>${ht?'Seksyon':'Section'}:</b> ${esc(student?.section||'—')}</div></div><table><thead><tr><th>${ht?'Matiyè':'Matière'}</th><th>${ht?'Koefisyan':'Coefficient'}</th><th>${ht?'Mwayèn':'Moyenne'}</th><th>${ht?'Pwen pondéré':'Points pondérés'}</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.name)}</td><td>${format(r.coefficient)}</td><td>${format(r.avg)}%</td><td>${format(r.points)}</td></tr>`).join('')}</tbody></table><div class="general"><span>${ht?'Mwayèn jeneral pondérée':'Moyenne générale pondérée'}</span><span>${weighted===null?'—':format(weighted)+'%'}</span></div><div class="validation"><div class="validation-title">${ht?'Validasyon dokiman':'Validation du document'}</div><div class="validation-grid"><div><div class="validation-field"></div><div class="validation-label">${ht?'Siyati Direksyon':'Signature de la Direction'}</div></div><div><div class="validation-field"></div><div class="validation-label">${ht?'Kachè lekòl la':'Cachet de l’école'}</div></div><div class="date-field"><div class="validation-field" style="padding-top:28px"></div><div class="validation-label">${ht?'Dat':'Date'}</div></div></div></div><div class="footer">${ht?'Dokiman akademik ofisyèl':'Document académique officiel'}</div></body></html>`
