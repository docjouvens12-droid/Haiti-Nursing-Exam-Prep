...
export default function OfficialRecordPrintEnhancer(){
 useEffect(()=>{
  const handler=async(event:Event)=>{
   ...
   const gi=allStrong.findIndex(x=>x==='Mwayèn jeneral'||x==='Moyenne générale'),general=gi>=0?(allStrong[gi+1]||'—'):'—'
   const docTitle=ht?'RELVE NÒT':'RELEVÉ DE NOTES'
   const html=`<!doctype html><html><head><meta charset="utf-8"><title>${docTitle}</title><style>@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#13213a;margin:0;font-size:12px}.header{text-align:center;border-bottom:3px double #123f68;padding-bottom:12px;margin-bottom:16px}.logo{max-width:86px;max-height:86px;object-fit:contain;margin-bottom:7px}.school{font-size:22px;font-weight:900;color:#0f4c81}.contact{font-size:10px;margin-top:4px;color:#5e6a78}.doc-title{text-align:center;font-size:20px;font-weight:900;margin:10px 0 14px}.student{border:1px solid #9fb4c8;padding:10px 12px;display:grid;grid-template-columns:1fr 1fr;gap:7px 18px;margin-bottom:14px;background:#f8fafc}.period{display:flex;justify-content:space-between;background:#eaf2f8;border:1px solid #bdccda;padding:8px 10px;margin-top:11px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #cbd7e1;padding:6px 8px}th{text-align:left;background:#f5f8fb}th:last-child,td:last-child{text-align:right;width:22%}.empty{border:1px solid #cbd7e1;border-top:0;padding:7px;color:#667}.general{margin-top:16px;border:2px solid #0f4c81;padding:10px 12px;display:flex;justify-content:space-between;font-size:16px;font-weight:900}.signatures{display:grid;grid-template-columns:1fr 1fr;gap:60px;margin-top:48px;text-align:center}.line{border-top:1px solid #333;padding-top:6px}.stamp{height:58px;margin-bottom:4px}.footer{text-align:center;margin-top:24px;color:#667;font-size:9px}</style></head><body><div class="header">${logo}<div class="school">${escapeHtml(schoolName)}</div>${contact?`<div class="contact">${escapeHtml(contact)}</div>`:''}</div><div class="doc-title">${docTitle} — ${escapeHtml(type)}</div><div class="student"><div><b>${ht?'Elèv':'Élève'}:</b> ${escapeHtml(student)}</div><div><b>${ht?'Ane akademik':'Année scolaire'}:</b> ${escapeHtml(year)}</div><div><b>${ht?'Klas / Nivo':'Classe / Niveau'}:</b> ${escapeHtml(level)}</div><div><b>${ht?'Seksyon':'Section'}:</b> ${escapeHtml(section)}</div></div>${periods}<div class="general"><span>${ht?'Mwayèn jeneral':'Moyenne générale'}</span><span>${escapeHtml(general)}</span></div><div class="signatures"><div><div class="stamp"></div><div class="line">${ht?'Siyati Direksyon':'Signature de la Direction'}</div></div><div><div class="stamp">${ht?'Kachè lekòl la':'Cachet de l’école'}</div><div class="line">${ht?'Kachè / Dat':'Cachet / Date'}</div></div></div><div class="footer">${ht?'Dokiman akademik ofisyèl':'Document académique officiel'}</div></body></html>`
