...
export default function StudentAccessPanel(){
 ...
 const [usedStudentIds,setUsedStudentIds]=useState<string[]>([])
 ...
 const load=async(u:User)=>{
  ...
  const [sr,pr]=await Promise.all([
   ...
   supabase.from('school_profiles').select('student_id').eq('role','student')
  ])
  ...
  setUsedStudentIds((pr.data||[]).map(x=>x.student_id).filter(Boolean))
 }
