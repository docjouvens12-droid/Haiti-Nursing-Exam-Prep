...
export default function StudentTrimesterSummary(){
 ...
 useEffect(()=>{
  const attach=()=>{
   ...
   if(bulletin){let finalMount=menu?.querySelector('[data-student-final-report-mount]') as HTMLElement|null;if(!finalMount){finalMount=document.createElement('div');finalMount.setAttribute('data-student-final-report-mount','true');finalMount.style.display='contents';bulletin.insertAdjacentElement('afterend',finalMount)}setFinalTarget(finalMount)}
  }
 ...
 const finalGroups=useMemo(()=>{const count=finalType==='trimester'?3:4;return Array.from({length:count},(_,i)=>i+1).map(number=>{const rows=grades.filter(g=>matchesSelection(g.term,finalType,number));return {number,rows,avg:average(rows)}})},[grades,finalType])
 ...
 const finalReportHtml=()=>{
  ...
  const sections=finalGroups.map(group=>{const rows=group.rows.length?`<table><thead><tr><th>${ht?'Matiyè':'Matière'}</th><th>${ht?'Nòt':'Note'}</th></tr></thead><tbody>${group.rows.map(g=>`<tr><td>${escapeHtml(g.subject)}</td><td>${g.score}%</td></tr>`).join('')}</tbody></table>`:`<p class="empty">${ht?'Pa gen nòt pibliye.':'Aucune note publiée.'}</p>`;return `<section><div class="section-title"><strong>${typeLabel} ${group.number}</strong><strong>${group.avg===null?'—':`${ht?'Mwayèn':'Moyenne'}: ${group.avg}%`}</strong></div>${rows}</section>`}).join('')
  const title=ht?'Relve nòt mwen':'Mon relevé de notes'
  return `<!doctype html><html><head><meta charset="utf-8"><title>${title}</title><style>@page{size:A4;margin:12mm}body{font-family:Arial,sans-serif;color:#182433;margin:0;font-size:12px}h1{text-align:center;color:#0f4c81;margin:0 0 4px;font-size:22px}.school{text-align:center;font-weight:700;margin-bottom:14px}.student{border:1px solid #cfd9e3;padding:9px 12px;margin-bottom:12px;display:grid;grid-template-columns:1fr 1fr;gap:5px 18px}.section-title{display:flex;justify-content:space-between;background:#eef4f8;padding:7px 9px;border:1px solid #d8e2ea;margin-top:9px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #d8e2ea;padding:5px 7px;text-align:left}th:last-child,td:last-child{text-align:right}.general{margin-top:14px;border-top:2px solid #0f4c81;padding-top:9px;display:flex;justify-content:space-between;font-size:16px;font-weight:700}</style></head><body><div class="school">PORTAIL SCOLAIRE HAÏTI</div><h1>${title} — ${typeLabel}</h1><div class="student"><div><strong>${ht?'Elèv':'Élève'}:</strong> ${escapeHtml(student?.name||'—')}</div><div><strong>${ht?'Ane akademik':'Année scolaire'}:</strong> ${escapeHtml(student?.year||'—')}</div><div><strong>${ht?'Klas / Nivo':'Classe / Niveau'}:</strong> ${escapeHtml(student?.level||'—')}</div><div><strong>${ht?'Seksyon':'Section'}:</strong> ${escapeHtml(student?.section||'—')}</div></div>${sections}<div class="general"><span>${ht?'Mwayèn jeneral':'Moyenne générale'}</span><span>${generalAverage===null?'—':generalAverage+'%'}</span></div></body></html>`
 }
 ...
 const summary=createPortal(<div style={{marginBottom:14}}><div style={{border:'1px solid #dde6ef',borderRadius:14,padding:14,background:'#f8fafc'}}><div style={{fontWeight:800,marginBottom:10}}>{ht?'Chwazi rezilta pou wè':'Choisir le résultat à afficher'}</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}><div><label>{ht?'Kalite':'Type'}</label><select value={assessmentType} onChange={e=>setAssessmentType(e.target.value as AssessmentType)}><option value="trimester">{ht?'Trimès':'Trimestre'}</option><option value="control">{ht?'Kontwòl':'Contrôle'}</option></select></div><div><label>{ht?'Nimewo':'Numéro'}</label><select value={assessmentNumber} onChange={e=>setAssessmentNumber(Number(e.target.value))}>{(assessmentType==='trimester'?[1,2,3]:[1,2,3,4]).map(n=><option key={n} value={n}>{n}</option>)}</select></div></div><div style={{display:'flex',justifyContent:'space-between',borderTop:'1px solid #dde6ef',paddingTop:12}}><span style={{fontWeight:700}}>{selectedLabel}</span><strong style={{fontSize:24,color:'#0f4c81'}}>{selectedAverage===null?'—':selectedAverage+'%'}</strong></div></div><div style={{marginTop:10,border:'1px solid #dde6ef',borderRadius:14,padding:14,background:'#fff',display:'flex',justifyContent:'space-between'}}><strong>{ht?'Mwayèn jeneral':'Moyenne générale'}</strong><strong style={{fontSize:24,color:'#0f4c81'}}>{generalAverage===null?'—':generalAverage+'%'}</strong></div></div>,target)

 const finalReport=finalTarget?createPortal(<><button className="menuBtn" type="button" onClick={()=>setShowFinal(v=>!v)}>📑 {ht?'Relve nòt mwen':'Mon relevé de notes'}</button>{showFinal&&<div className="card" style={{marginTop:12,gridColumn:'1 / -1'}}><h2>{ht?'Relve nòt mwen':'Mon relevé de notes'}</h2><div style={{border:'1px solid #dde6ef',borderRadius:12,padding:12,background:'#f8fafc',display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:14}}><div><strong>{ht?'Elèv':'Élève'}:</strong> {student?.name||'—'}</div><div><strong>{ht?'Ane akademik':'Année scolaire'}:</strong> {student?.year||'—'}</div><div><strong>{ht?'Klas / Nivo':'Classe / Niveau'}:</strong> {student?.level||'—'}</div><div><strong>{ht?'Seksyon':'Section'}:</strong> {student?.section||'—'}</div></div><div style={{marginBottom:14}}><label>{ht?'Kalite':'Type'}</label><select value={finalType} onChange={e=>setFinalType(e.target.value as AssessmentType)}><option value="trimester">{ht?'Trimès':'Trimestre'}</option><option value="control">{ht?'Kontwòl':'Contrôle'}</option></select></div><div style={{display:'grid',gap:14}}>{finalGroups.map(group=><div key={`${finalType}-${group.number}`} style={{border:'1px solid #dde6ef',borderRadius:12,padding:12}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}><strong>{finalType==='trimester'?(ht?'Trimès':'Trimestre'):(ht?'Kontwòl':'Contrôle')} {group.number}</strong><strong>{group.avg===null?'—':`${ht?'Mwayèn':'Moyenne'} ${group.avg}%`}</strong></div>{group.rows.length===0?<div className="muted">{ht?'Pa gen nòt pibliye.':'Aucune note publiée.'}</div>:<table><tbody>{group.rows.map((g,index)=><tr key={`${group.number}-${g.subject}-${index}`}><td>{g.subject}</td><td className="score">{g.score}%</td></tr>)}</tbody></table>}</div>)}</div><div style={{marginTop:14,paddingTop:12,borderTop:'2px solid #dde6ef',display:'flex',justifyContent:'space-between'}}><strong>{ht?'Mwayèn jeneral':'Moyenne générale'}</strong><strong style={{fontSize:24,color:'#0f4c81'}}>{generalAverage===null?'—':generalAverage+'%'}</strong></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginTop:16}}><button type="button" className="btn secondary" onClick={downloadWord}>⬇️ {ht?'Telechaje Word':'Télécharger Word'}</button><button type="button" className="btn" onClick={printFinalReport}>🖨️ {ht?'Enprime':'Imprimer'}</button></div></div>}</>,finalTarget):null
