 ...
 useEffect(()=>{
  const apply=()=>{
   document.querySelectorAll('.card').forEach(cardNode=>{
    ...
    if(card.querySelector('[data-native-validation]')){
      card.querySelector('[data-record-validation-fields]')?.remove()
      return
    }
    ...
    box.innerHTML=`<div data-validation-title style="font-size:16px;font-weight:900;color:#0f4c81;margin-bottom:18px">${ht?'Validasyon dokiman':'Validation du document'}</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:22px 26px"><div><div style="height:52px"></div><div data-validation-signature style="border-top:1px solid #374151;padding-top:7px;font-weight:800">${ht?'Siyati Direksyon':'Signature de la Direction'}</div></div><div><div style="height:52px"></div><div data-validation-stamp style="border-top:1px solid #374151;padding-top:7px;font-weight:800">${ht?'Kachè lekòl la':'Cachet de l’école'}</div></div><div style="grid-column:1/-1"><div style="height:28px"></div><div data-validation-date style="border-top:1px solid #374151;padding-top:7px;font-weight:800">${ht?'Dat':'Date'}</div></div></div>`
    const actionWrap=actionButton.parentElement
    if(actionWrap&&actionWrap.parentElement)actionWrap.parentElement.insertBefore(box,actionWrap)
  ...
  return()=>{observer.disconnect();window.clearInterval(timer);window.removeEventListener('focus',apply)}
 },[])
 return null
}
