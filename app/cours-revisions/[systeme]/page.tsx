...
export default async function SystemeCoursPage({ params }: { params: Promise<{ systeme: string }> }) {
  ...
  const systeme = getAutreSysteme(slug);
  if (!systeme) notFound();
  ...
  return (
    <main style={{ maxWidth: 920, margin: "0 auto", padding: "26px 18px 90px" }}>
      ...
      <header style={{ margin: "18px 0 22px", background: "linear-gradient(135deg,#071b4f,#1748b7)", color: "white", borderRadius: 22, padding: "28px 24px" }}>
        ...
        <p style={{ margin: 0, lineHeight: 1.6, opacity: 0.92 }}>Rappel essentiel, pathologies prioritaires et prise en charge infirmière structurée.</p>
      </header>

      <section style={{ background: "#eef6ff", border: "1px solid #cfe3fb", borderRadius: 18, padding: 20, marginBottom: 22 }}>
        <h2 style={{ color: "#0b1f59", margin: "0 0 8px", fontSize: 20 }}>Objectifs du module</h2>
        <p style={{ margin: 0, color: "#475569", lineHeight: 1.7 }}>{systeme.objectifs}</p>
      </section>
          ...
          <article key={pathologie.nom} style={{ background: "white", border: "1px solid #dfe6f0", borderRadius: 19, overflow: "hidden", boxShadow: "0 6px 20px rgba(11,31,89,.05)" }}>
            ...
            <div style={{ padding: "8px 20px 18px" }}>
              {rubriques.map(([cle, label]) => (
                <section key={cle} style={{ padding: "13px 0", borderBottom: cle === "points" ? "none" : "1px solid #edf1f6" }}>
              ...
              <aside style={{ background: "#eef6ff", border: "1px solid #cfe3fb", borderRadius: 14, padding: 14, marginTop: 8 }}>
                ...
                <p style={{ margin: "6px 0 0", color: "#475569", lineHeight: 1.65, fontSize: 14 }}>{pathologie.priorite}</p>
              </aside>
            </div>
          </article>
        ))}
      ...
      <section style={{ background: "#f8fafc", border: "1px solid #dfe6f0", borderRadius: 18, padding: 20, marginTop: 22 }}>
        ...
        <ol style={{ margin: 0, paddingLeft: 20, display: "grid", gap: 10 }}>
          {systeme.references.map((reference) => (
            <li key={reference.href} style={{ color: "#334155", lineHeight: 1.55, fontSize: 13 }}>
              <a href={reference.href} target="_blank" rel="noreferrer" style={{ color: "#1748b7", fontWeight: 700, textDecoration: "underline" }}>
                {reference.label}
              </a>
            </li>
          ))}
      ...
      <p style={{ marginTop: 22, color: "#718096", fontSize: 12, lineHeight: 1.55 }}>
        Contenu éducatif de révision. Les protocoles et recommandations cliniques peuvent évoluer.
      </p>
    </main>
  );
}
