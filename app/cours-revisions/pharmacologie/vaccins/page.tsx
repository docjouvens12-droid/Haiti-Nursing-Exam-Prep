...
export default async function Page(){
  ...
  return <main style={{maxWidth:940,margin:'0 auto',padding:'26px 18px 90px'}}>
    ...
    <header style={{margin:'18px 0 22px',background:'linear-gradient(135deg,#14532d,#16a34a)',color:'white',borderRadius:22,padding:'28px 24px'}}>
      <div style={{opacity:.84,fontSize:13,fontWeight:900,letterSpacing:1}}>PHARMACOLOGIE • IMMUNOLOGIE</div>
      ...
      <div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:16}}>
        <span style={{background:'rgba(255,255,255,.14)',padding:'7px 10px',borderRadius:999,fontSize:13,fontWeight:800}}>⏱️ {module.estimated_minutes} min</span>
        <span style={{background:'rgba(255,255,255,.14)',padding:'7px 10px',borderRadius:999,fontSize:13,fontWeight:800}}>📘 {(sections??[]).length} sections</span>
        <span style={{background:'rgba(255,255,255,.14)',padding:'7px 10px',borderRadius:999,fontSize:13,fontWeight:800}}>📝 {(quiz??[]).length} questions</span>
      </div>
    </header>

    <section style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:18,padding:20,marginBottom:22}}>
      <h2 style={{color:'#166534',margin:'0 0 10px',fontSize:21}}>Objectifs d’apprentissage</h2>
    ...
    <section style={{background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:18,padding:20,marginBottom:22}}>
      ...
      <p style={{margin:0,color:'#475569',lineHeight:1.72}}>Avant toute vaccination, vérifier les contre-indications, la grossesse, le niveau d’immunodépression et la chaîne du froid. Après administration, savoir reconnaître immédiatement une anaphylaxie et utiliser l’épinéphrine IM sans retarder l’appel aux secours.</p>
    </section>

    <div style={{display:'grid',gap:14}}>{((sections??[]) as Section[]).map(section=><article key={section.id} style={{background:'white',border:'1px solid #e3e9ef',borderRadius:17,padding:'19px 20px',boxShadow:'0 5px 18px rgba(20,83,45,.04)'}}><h2 style={{fontSize:20,color:'#166534',margin:'0 0 8px'}}>{section.title}</h2><p style={{margin:0,color:'#475569',lineHeight:1.72,whiteSpace:'pre-line'}}>{section.content}</p></article>)}</div>

    <ModuleQuiz questions={(quiz??[]) as QuizQuestion[]} />

    <section style={{marginTop:22,background:'#f8fafc',border:'1px solid #dfe6f0',borderRadius:16,padding:18}}><h2 style={{fontSize:18,color:'#0b1f59',margin:'0 0 8px'}}>Références principales</h2><p style={{margin:0,color:'#64748b',lineHeight:1.65}}>Contenu conçu à partir des principes actuels de sécurité vaccinale, des bonnes pratiques d’administration et de chaîne du froid. Les calendriers et protocoles nationaux en vigueur restent prioritaires pour les indications propres à chaque vaccin.</p></section>
  </main>;
}
