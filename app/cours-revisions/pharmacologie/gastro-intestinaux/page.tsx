...
export default async function Page(){
  ...
  const {data:module,error:moduleError}=await supabase.from('learning_modules').select('id,title,summary,estimated_minutes,learning_objectives').eq('slug','gastro-intestinal-et-renal').eq('is_published',true).maybeSingle();
  ...
  return <main style={{maxWidth:940,margin:'0 auto',padding:'26px 18px 90px'}}>
    ...
    <header style={{margin:'18px 0 22px',background:'linear-gradient(135deg,#78350f,#d97706)',color:'white',borderRadius:22,padding:'28px 24px'}}>
      <div style={{opacity:.84,fontSize:13,fontWeight:900,letterSpacing:1}}>PHARMACOLOGIE • GASTRO-INTESTINAL & RÉNAL</div>
    ...
    <section style={{background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:18,padding:20,marginBottom:22}}>
      ...
      <p style={{margin:0,color:'#475569',lineHeight:1.72}}>Hémorragie digestive avec instabilité, abdomen aigu, anurie, hyperkaliémie sévère, déshydratation importante ou détérioration rapide de la fonction rénale nécessitent une évaluation urgente.</p>
    </section>
    ...
    <section style={{marginTop:22,background:'#f8fafc',border:'1px solid #dfe6f0',borderRadius:16,padding:18}}><h2 style={{fontSize:18,color:'#0b1f59',margin:'0 0 8px'}}>Références principales</h2><p style={{margin:0,color:'#64748b',lineHeight:1.65}}>Contenu aligné sur les principes actuels de pharmacologie digestive et de sécurité rénale, ainsi que sur les recommandations contemporaines concernant la maladie rénale chronique et l’utilisation des inhibiteurs de SGLT2 chez certains patients.</p></section>
  </main>;
}
