...
export default async function Page(){
  ...
  const {data:module,error:moduleError}=await supabase.from('learning_modules').select('id,title,summary,estimated_minutes,learning_objectives').eq('slug','medicaments-urgence-antidotes').eq('is_published',true).maybeSingle();
  ...
  return <main style={{maxWidth:940,margin:'0 auto',padding:'26px 18px 90px'}}>
    ...
    <header style={{margin:'18px 0 22px',background:'linear-gradient(135deg,#7f1d1d,#b91c1c)',color:'white',borderRadius:22,padding:'28px 24px'}}>
      <div style={{opacity:.84,fontSize:13,fontWeight:900,letterSpacing:1}}>PHARMACOLOGIE • URGENCES</div>
    ...
    <section style={{background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:18,padding:20,marginBottom:22}}>
      ...
      <p style={{margin:0,color:'#475569',lineHeight:1.72}}>En urgence, aucun antidote ne doit retarder l’ABC, la ventilation, la RCP, la défibrillation, le contrôle d’un saignement ou l’appel d’aide lorsqu’ils sont indiqués.</p>
    </section>

    <div style={{display:'grid',gap:14}}>{((sections??[]) as Section[]).map(section=><article key={section.id} style={{background:'white',border:'1px solid #e3e9ef',borderRadius:17,padding:'19px 20px',boxShadow:'0 5px 18px rgba(127,29,29,.04)'}}><h2 style={{fontSize:20,color:'#991b1b',margin:'0 0 8px'}}>{section.title}</h2><p style={{margin:0,color:'#475569',lineHeight:1.72,whiteSpace:'pre-line'}}>{section.content}</p></article>)}</div>

    <ModuleQuiz questions={(quiz??[]) as QuizQuestion[]} />

    <section style={{marginTop:22,background:'#f8fafc',border:'1px solid #dfe6f0',borderRadius:16,padding:18}}><h2 style={{fontSize:18,color:'#0b1f59',margin:'0 0 8px'}}>Références principales</h2><p style={{margin:0,color:'#64748b',lineHeight:1.65}}>Contenu de révision aligné sur les principes actuels de réanimation et de toxicologie d’urgence. Les protocoles locaux, le centre antipoison et les prescriptions institutionnelles restent prioritaires.</p></section>
  </main>;
}
