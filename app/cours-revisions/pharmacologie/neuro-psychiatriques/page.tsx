...
export default async function Page(){
  ...
  const {data:module,error:moduleError}=await supabase.from('learning_modules').select('id,title,summary,estimated_minutes,learning_objectives').eq('slug','neurologie-psychiatrie').eq('is_published',true).maybeSingle();
  ...
  return <main style={{maxWidth:940,margin:'0 auto',padding:'26px 18px 90px'}}>
    ...
    <header style={{margin:'18px 0 22px',background:'linear-gradient(135deg,#4c1d95,#7c3aed)',color:'white',borderRadius:22,padding:'28px 24px'}}>
      <div style={{opacity:.84,fontSize:13,fontWeight:900,letterSpacing:1}}>PHARMACOLOGIE • NEUROLOGIE & PSYCHIATRIE</div>
    ...
    <section style={{background:'#f5f3ff',border:'1px solid #ddd6fe',borderRadius:18,padding:20,marginBottom:22}}>
      <h2 style={{color:'#6d28d9',margin:'0 0 10px',fontSize:21}}>Objectifs d’apprentissage</h2>
    ...
    <section style={{background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:18,padding:20,marginBottom:22}}>
      ...
      <p style={{margin:0,color:'#475569',lineHeight:1.72}}>Convulsion prolongée, éruption sévère sous lamotrigine, idées suicidaires, rigidité avec fièvre, clonus avec agitation, dépression respiratoire ou signes de toxicité au lithium nécessitent une évaluation rapide.</p>
    </section>

    <div style={{display:'grid',gap:14}}>{((sections??[]) as Section[]).map(section=><article key={section.id} style={{background:'white',border:'1px solid #e3e9ef',borderRadius:17,padding:'19px 20px',boxShadow:'0 5px 18px rgba(76,29,149,.04)'}}><h2 style={{fontSize:20,color:'#5b21b6',margin:'0 0 8px'}}>{section.title}</h2><p style={{margin:0,color:'#475569',lineHeight:1.72}}>{section.content}</p></article>)}</div>

    <ModuleQuiz questions={(quiz??[]) as QuizQuestion[]}/>

    <section style={{marginTop:22,background:'#f8fafc',border:'1px solid #dfe6f0',borderRadius:16,padding:18}}><h2 style={{fontSize:18,color:'#0b1f59',margin:'0 0 8px'}}>Références principales</h2><p style={{margin:0,color:'#64748b',lineHeight:1.65}}>Contenu pédagogique fondé sur les principes actuels de pharmacologie neurologique et psychiatrique, notamment les recommandations de l’American Academy of Neurology pour les traitements antiépileptiques et les principes de surveillance clinique et de sécurité des psychotropes. Les protocoles locaux et prescriptions individuelles restent prioritaires.</p></section>
  </main>;
}
