...
export default async function Page(){
  ...
  const { data: module, error: moduleError } = await supabase
    ...
    .eq('slug','pharmacologie-obstetricale')
  ...
  return <main style={{maxWidth:940,margin:'0 auto',padding:'26px 18px 90px'}}>
    ...
    <header style={{margin:'18px 0 22px',background:'linear-gradient(135deg,#7c2d12,#ea580c)',color:'white',borderRadius:22,padding:'28px 24px'}}>
      <div style={{opacity:.84,fontSize:13,fontWeight:900,letterSpacing:1}}>PHARMACOLOGIE • OBSTÉTRIQUE</div>
    ...
    <section style={{background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:18,padding:20,marginBottom:22}}>
      <h2 style={{color:'#9a3412',margin:'0 0 10px',fontSize:21}}>Objectifs d’apprentissage</h2>
    ...
    <section style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:18,padding:20,marginBottom:22}}>
      ...
      <p style={{margin:0,color:'#475569',lineHeight:1.72}}>Sous traitement obstétrical, convulsion, hémorragie importante, hypertension sévère, dépression respiratoire, tachysystolie avec anomalies fœtales ou altération de conscience nécessitent une réponse immédiate selon le protocole local.</p>
    </section>

    <div style={{display:'grid',gap:14}}>{((sections??[]) as Section[]).map(section=><article key={section.id} style={{background:'white',border:'1px solid #e3e9ef',borderRadius:17,padding:'19px 20px',boxShadow:'0 5px 18px rgba(124,45,18,.04)'}}><h2 style={{fontSize:20,color:'#9a3412',margin:'0 0 8px'}}>{section.title}</h2><p style={{margin:0,color:'#475569',lineHeight:1.72}}>{section.content}</p></article>)}</div>

    <ModuleQuiz questions={(quiz??[]) as QuizQuestion[]} />

    <section style={{marginTop:22,background:'#f8fafc',border:'1px solid #dfe6f0',borderRadius:16,padding:18}}><h2 style={{fontSize:18,color:'#0b1f59',margin:'0 0 8px'}}>Références principales</h2><p style={{margin:0,color:'#64748b',lineHeight:1.65}}>Contenu conçu pour la révision infirmière et la sécurité médicamenteuse obstétricale. Les protocoles locaux, les prescriptions et les recommandations institutionnelles restent prioritaires.</p></section>
  </main>;
}
