...
export default async function Page(){
  ...
  const {data:module,error:moduleError}=await supabase.from('learning_modules').select('id,slug,title,summary,category,estimated_minutes,learning_objectives').eq('slug','securite-medicamenteuse').eq('is_published',true).maybeSingle();
  ...
  return <main style={{maxWidth:940,margin:'0 auto',padding:'26px 18px 90px'}}>
    ...
    <header style={{margin:'18px 0 22px',background:'linear-gradient(135deg,#0b3b33,#0f766e)',color:'white',borderRadius:22,padding:'28px 24px'}}>
      <div style={{opacity:.82,fontSize:13,fontWeight:900,letterSpacing:1}}>MODULE TRANSVERSAL</div>
      <h1 style={{margin:'7px 0 8px',fontSize:34}}>{module.title}</h1>
      <p style={{margin:0,lineHeight:1.65,opacity:.95,maxWidth:760}}>{module.summary}</p>
    ...
    <section style={{background:'#ecfdf5',border:'1px solid #bbf7d0',borderRadius:18,padding:20,marginBottom:22}}>
