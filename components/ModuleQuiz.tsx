...
type Props={questions:QuizQuestion[];description?:string};

export default function ModuleQuiz({questions,description='QCM directs et cas cliniques. La correction détaillée apparaît après chaque réponse.'}:Props){
  ...
  return <section style={{background:'white',border:'1px solid #dfe7ec',borderRadius:20,padding:22,marginTop:24,boxShadow:'0 8px 26px rgba(11,31,89,.06)'}}>
    <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',marginBottom:15}}><strong style={{color:'#0f766e'}}>Question {index+1}/{questions.length}</strong><span style={{fontSize:12,fontWeight:800,color:'#64748b'}}>{current.question_type==='case'?'Cas clinique':'QCM'}</span></div>
